...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        ...
        echo -e "\033[0;31m"
        figlet Hackops-Academy
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mHackops-Academy \e[0m"
    else
        echo -e "\033[0;32m"
        toilet -f mono12 -F border C-Strom
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN C-Strom Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
